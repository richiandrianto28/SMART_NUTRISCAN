...
def render_holistic_nutrition_metrics(energi, takaran_saji, lemak_total, karbohidrat, protein, gula, natrium, lemak_jenuh, current_threshold, user_profile):
    ...
    with metrik_col1:
        ...
        if kepadatan_energi > 4:
            kepadatan_status, kepadatan_color = "🔴 Sangat Tinggi (Padat Kalori)", "inverse"
        elif kepadatan_energi > 1.5:
            kepadatan_status, kepadatan_color = "🟡 Tinggi", "off"
        elif kepadatan_energi > 0.6:
            kepadatan_status, kepadatan_color = "🟢 Rendah (Ideal)", "normal"
        else:
            kepadatan_status, kepadatan_color = "🔵 Sangat Rendah", "normal"
    ...
    with metrik_col2:
        ...
        if rasio_gula > 50:
            rasio_status, rasio_color = "🔴 Tinggi Gula Sederhana", "inverse"
        elif rasio_gula > 25:
            rasio_status, rasio_color = "🟡 Waspada Glikemik", "off"
        else:
            rasio_status, rasio_color = "🟢 Karbohidrat Kompleks", "normal"
    ...
    if total_kalori_makro > 0:
        ...
        fig_makro = px.pie(df_makro, values="Kalori", names="Sumber", hole=0.45, color="Sumber",
                           color_discrete_map={"Lemak (9 kkal/g)": "#EF553B", "Karbohidrat (4 kkal/g)": "#00CC96", "Protein (4 kkal/g)": "#636EFA"},
                           title="Distribusi Sumber Kalori (Macronutrient Split)", hover_data=['Gram'])
    ...
    if pct_gula > 50: st.warning(f"⚠️ 1 Porsi produk ini menghabiskan **{pct_gula:.1f}%** jatah gula harian Anda!")
    ...
    if pct_natrium > 50: st.warning(f"⚠️ 1 Porsi produk ini menghabiskan **{pct_natrium:.1f}%** jatah natrium harian Anda!")
...
def generate_html_report(product_name, risk_score, recommendation, upf_ingredients, nutrition_data, tdee_profile, kepadatan_energi, takaran_saji):
    ...
    <body>
        ...
        <div class="section">
            ...
            <table>
                <tr><th>Nutrisi</th><th>Kandungan per Saji</th><th>% Pemenuhan Batas Harian Profil</th></tr>
                <tr><td>Gula</td><td>{nutrition_data.get('gula', 0)} g</td><td>{((nutrition_data.get('gula', 0) / tdee_profile['gula']) * 100):.1f}%</td></tr>
                <tr><td>Natrium</td><td>{nutrition_data.get('natrium', 0)} mg</td><td>{((nutrition_data.get('natrium', 0) / tdee_profile['natrium']) * 100):.1f}%</td></tr>
                <tr><td>Lemak Jenuh</td><td>{nutrition_data.get('lemak_jenuh', 0)} g</td><td>{((nutrition_data.get('lemak_jenuh', 0) / tdee_profile['lemak_jenuh']) * 100):.1f}%</td></tr>
            </table>
        </div>
        <div class="footer"><p>Laporan ini digenerate secara otomatis oleh model Hybrid CBLIGHT-WOA & BI Analytics.</p></div>
    </body>
    </html>
    """
    return html_content
...
def hitung_tdee_dinamis(gender, usia, berat, tinggi, aktivitas):
    ...
    faktor = {"Sedentary (Jarang Olahraga)": 1.2, "Ringan (Olahraga 1-3x/minggu)": 1.375, "Sedang (Olahraga 3-5x/minggu)": 1.55, "Aktif (Olahraga 6-7x/minggu)": 1.725, "Sangat Aktif (Pekerja Fisik / Atlet)": 1.9}
    tdee = bmr * faktor.get(aktivitas, 1.2)
    max_gula_g = (tdee * 0.10) / 4
    max_lemak_jenuh_g = (tdee * 0.10) / 9
    return {"kalori": tdee, "gula": max_gula_g, "lemak_jenuh": max_lemak_jenuh_g, "natrium": 2000}
...
with st.sidebar:
    ...
    with col_g: user_gender = st.selectbox("Gender", ["Pria", "Wanita"])
    with col_u: user_age = st.number_input("Usia (Tahun)", min_value=1, max_value=120, value=25)

    col_w, col_h = st.columns(2)
    with col_w: user_weight = st.number_input("Berat (kg)", min_value=10.0, max_value=300.0, value=65.0)
    with col_h: user_height = st.number_input("Tinggi (cm)", min_value=50.0, max_value=250.0, value=165.0)

    user_activity = st.selectbox("Tingkat Aktivitas", ["Sedentary (Jarang Olahraga)", "Ringan (Olahraga 1-3x/minggu)", "Sedang (Olahraga 3-5x/minggu)", "Aktif (Olahraga 6-7x/minggu)", "Sangat Aktif (Pekerja Fisik / Atlet)"])
    ...
    if kondisi_medis == "Penderita Hipertensi": calculated_threshold["natrium"] = 1200
    elif kondisi_medis == "Risiko Penyakit Ginjal": calculated_threshold["natrium"] = 1000; calculated_threshold["kalori"] *= 0.9
    elif kondisi_medis == "Anak-anak (Pre-set)": calculated_threshold["gula"] = 25; calculated_threshold["natrium"] = 1500
    ...
    st.markdown("---")
    app_mode = st.radio("Pilih Fitur:", ["Analisis Produk Tunggal", "Scan from Image", "Analisis Batch (Excel)", "Perbandingan Produk", "Simulasi Konsumsi", "Riwayat Analisis", "Edukasi Gizi"])
    st.markdown("---")
...
if app_mode == "Analisis Produk Tunggal":
    ...
    if not all([feat_model, lgbm_model, w2v_model, scaler]):
        ...
        with main_col:
            ...
            energi = c1.number_input("Energi (kkal)", min_value=0.0, value=180.0)
            ...
            natrium = c8.number_input("Natrium (mg)", min_value=0.0, value=200.0)
        ...
        with right_col:
            ...
            if analyze_button:
                ...
                # ==========================================================================
                # 🌟 MARKER FITUR 2: HASIL KLASIFIKASI (VALIDASI ZERO-INPUT & CLASSIFY_RISK)
                # ==========================================================================
                if not has_sufficient_input(nutrition_data):
                    ...
                    st.info("Sistem memblokir prediksi saat data kosong demi menjaga integritas model AI.")
                else:
                    with st.spinner('Menganalisis produk dengan model CBLIGHT-WOA...'):
                        ...
                        if risk_info["style"] == "success": st.success(f"Klasifikasi: {risk_info['label']}")
                        elif risk_info["style"] == "warning": st.warning(f"Klasifikasi: {risk_info['label']}")
                        else: st.error(f"Klasifikasi: {risk_info['label']}")
                        ...
                        fig_radar.add_trace(go.Scatterpolar(
                            r=norm_values + [norm_values[0]], theta=categories + [categories[0]], fill='toself',
                            name='Kandungan Produk', line_color='red' if risk_score > 50 else 'orange' if risk_score > 25 else 'green',
                            hovertemplate="Feature: %{theta}<br>Skor Relatif: %{r:.1f}/100<extra></extra>"
                        ))
                        fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100], showticklabels=False)), showlegend=False, margin=dict(l=20, r=20, t=20, b=20), height=250)
        ...
        if analyze_button and has_sufficient_input(nutrition_data):
            ...
            st.download_button(label="📥 Download Executive Health Report (HTML)", data=html_report, file_name=f"Health_Report_{product_name.replace(' ', '_')}.html", mime="text/html", type="primary")
    ...
    parsed_data = {
        'energi': 0.0, 'lemak_total': 0.0, 'lemak_jenuh': 0.0, 'protein': 0.0,
        'karbohidrat': 0.0, 'gula': 0.0, 'garam': 0.0, 'natrium': 0.0,
    ...
    with col_scan1:
        ...
        img_file_1 = st.file_uploader("Pilih foto Tabel Gizi...", type=["jpg", "jpeg", "png"], key="upload_1") if input_type_1 == "Upload File" else st.camera_input("Ambil foto Tabel Gizi langsung", key="cam_1")

        if img_file_1 is not None:
            image_1 = Image.open(img_file_1)
            st.image(image_1, caption="Gambar Gizi", use_container_width=True)
            
            # ==========================================================================
            # 🌟 MARKER FITUR 1: EKSEKUSI OCR GIZI (LAZY LOAD & SAFE EXECUTION)
            # ==========================================================================
            if st.button("Proses OCR Nilai Gizi", key="btn_ocr_1"):
                with st.spinner("Mengekstraksi Tabel Gizi dengan OCR Cerdas..."):
                    reader, reader_error = get_ocr_reader_safely()
                    if reader_error:
                        st.error(f"Gagal memuat OCR: {reader_error}")
                    else:
                        scan_result_1, ocr_error_1 = run_ocr_safely(reader, image_1, mode="nutrition")
                        if ocr_error_1:
                            st.error("OCR nilai gizi gagal diproses.")
                        else:
                            parsed_gizi = scan_result_1["parsed"]
                            for k, v in parsed_gizi.items():
                                if v != 0 and v != "Tidak terdeteksi." and v != "Produk Tanpa Nama" and v != "":
                                    parsed_data[k] = v
                            st.success("Tabel Gizi berhasil diproses secara aman!")
                            with st.expander("Lihat Hasil Mentah OCR Gizi", expanded=False):
                                st.text(scan_result_1.get("raw_text", "Tidak ada teks"))
            # ================= END MARKER FITUR 1 =================
    ...
    with col_scan2:
        ...
        img_file_2 = st.file_uploader("Pilih foto Teks Komposisi...", type=["jpg", "jpeg", "png"], key="upload_2") if input_type_2 == "Upload File" else st.camera_input("Ambil foto Teks Komposisi langsung", key="cam_2")
        ...
        if img_file_2 is not None:
            ...
            # ==========================================================================
            # 🌟 MARKER FITUR 1: EKSEKUSI OCR KOMPOSISI (LAZY LOAD & SAFE EXECUTION)
            # ==========================================================================
            if st.button("Proses OCR Komposisi", key="btn_ocr_2"):
                with st.spinner("Mengekstraksi Teks Komposisi dengan OCR Cerdas..."):
                    reader, reader_error = get_ocr_reader_safely()
                    if reader_error:
                        st.error(f"Gagal memuat OCR: {reader_error}")
                    else:
                        ...
                        if ocr_error_2:
                            st.error("OCR komposisi gagal diproses.")
                        else:
                            parsed_komposisi = scan_result_2["parsed"].get("komposisi", "Tidak terdeteksi.")
                            if parsed_komposisi != "Tidak terdeteksi.":
                                parsed_data['komposisi'] = parsed_komposisi
                            st.success("Teks Komposisi berhasil diproses!")
                            with st.expander("Lihat Hasil Mentah OCR Komposisi", expanded=False):
                                st.text(scan_result_2.get("raw_text", "Tidak ada teks"))
            # ================= END MARKER FITUR 1 =================
    ...
    if img_file_1 is not None or img_file_2 is not None:
        ...
        with main_col:
            ...
            c0, c1, c2 = st.columns(3)
            takaran_saji = c0.number_input("Takaran Saji (g/ml)", min_value=1.0, value=30.0, format="%.1f")
            energi = c1.number_input("Energi (kkal)", min_value=0.0, value=float(parsed_data.get('energi', 0)))
            lemak_total = c2.number_input("Lemak Total (g)", min_value=0.0, value=float(parsed_data.get('lemak_total', 0.0)), format="%.1f")

            c3, c4, c5 = st.columns(3)
            lemak_jenuh = c3.number_input("Lemak Jenuh (g)", min_value=0.0, value=float(parsed_data.get('lemak_jenuh', 0.0)), format="%.1f")
            protein = c4.number_input("Protein (g)", min_value=0.0, value=float(parsed_data.get('protein', 0.0)), format="%.1f")
            karbohidrat = c5.number_input("Karbohidrat (g)", min_value=0.0, value=float(parsed_data.get('karbohidrat', 0.0)), format="%.1f")

            c6, c7, c8, c9 = st.columns(4)
            gula = c6.number_input("Gula (g)", min_value=0.0, value=float(parsed_data.get('gula', 0.0)), format="%.1f")
            garam = c7.number_input("Garam (g)", min_value=0.0, value=float(parsed_data.get('garam', 0.0)), format="%.2f")
            natrium = c8.number_input("Natrium (mg)", min_value=0.0, value=float(parsed_data.get('natrium', 0)))
            natrium_benzoat = c9.number_input("Natrium Benzoat (mg)", min_value=0.0, value=float(parsed_data.get('natrium_benzoat', 0.0)), format="%.2f")
        ...
        if analyze_button and has_sufficient_input(nutrition_data):
            ...
            kepadatan_energi = energi / takaran_saji if takaran_saji > 0 else 0
            html_report = generate_html_report(product_name, risk_score, recommendation, upf_ingredients, nutrition_data, current_threshold, kepadatan_energi, takaran_saji)
            st.download_button(label="📥 Download Executive Health Report (HTML)", data=html_report, file_name=f"Health_Report_OCR_{product_name.replace(' ', '_')}.html", mime="text/html", type="primary")
    ...
    expected_columns = ['Energi', 'Lemak', 'Karbohidrat', 'Gula', 'Protein', 'Garam', 'Komposisi']
            ...
            if batch_analyze_button:
                ...
                with st.spinner(f"Menganalisis {total_rows} produk..."):
                    for i, row in df.iterrows():
                        nutrition_data = {
                            'energi': float(row.get('Energi', 0)), 'lemak_total': float(row.get('Lemak', 0)),
                            'karbohidrat': float(row.get('Karbohidrat', 0)), 'gula': float(row.get('Gula', 0)),
                            'protein': float(row.get('Protein', 0)), 'garam': float(row.get('Garam', 0)),
                            'natrium_benzoat': float(row.get('Natrium Benzoat', 0))
                        }
                        composition_text = row.get('Komposisi', "")

                        # ==========================================================================
                        # 🌟 MARKER FITUR 2: HASIL KLASIFIKASI BATCH (VALIDASI ZERO-INPUT & CLASSIFY_RISK)
                        # ==========================================================================
                        if not has_sufficient_input(nutrition_data):
                            results.append("Data Kosong")
                            results_labels.append("Gagal Analisis")
                        else:
                            risk_score, _, _ = analyze_product_fully(nutrition_data, composition_text, feat_model, lgbm_model, w2v_model, scaler)
                            risk_info = classify_risk(risk_score)
                            results.append(f"{risk_score:.2f}")
                            results_labels.append(risk_info["label"])
                        # ================= END MARKER FITUR 2 =================
                ...
                df_results = df.copy()
                df_results['Risk Score (%)'] = results
                df_results['Klasifikasi AI'] = results_labels
                
                st.subheader("Hasil Analisis Batch")
                st.dataframe(df_results)
                
                df_xlsx = to_excel(df_results)
                st.download_button(label="📥 Download Hasil Analisis (.xlsx)", data=df_xlsx, file_name="hasil_analisis_batch.xlsx")
    ...
    if 'product_a_data' not in st.session_state:
        st.session_state.product_a_data = {
            'name': 'Sereal Pagi A', 'takaran': 30.0, 'energi': 150.0, 'lemak_total': 5.0,
            'lemak_jenuh': 1.0, 'protein': 3.0, 'karbohidrat': 30.0, 'gula': 12.0,
            'natrium': 180.0, 'natrium_benzoat': 0.0, 'komposisi': 'Gandum Utuh, Gula, Garam.'
        }
    if 'product_b_data' not in st.session_state:
        st.session_state.product_b_data = {
            'name': 'Sereal Pagi B', 'takaran': 30.0, 'energi': 160.0, 'lemak_total': 6.0,
            'lemak_jenuh': 3.0, 'protein': 2.0, 'karbohidrat': 28.0, 'gula': 18.0,
            'natrium': 250.0, 'natrium_benzoat': 0.0, 'komposisi': 'Jagung, Gula, Sirup Fruktosa, Garam.'
        }
    ...
    with col1:
        ...
        if input_method_a == "Scan dengan Kamera":
            ...
            if img_file_a:
                ...
                with st.spinner("Memproses gambar Produk A..."):
                    # ==========================================================================
                    # 🌟 MARKER FITUR 1: EKSEKUSI OCR PRODUK A (LAZY LOAD & SAFE EXECUTION)
                    # ==========================================================================
                    reader, reader_error = get_ocr_reader_safely()
                    if reader_error:
                        st.error(f"Gagal memuat OCR: {reader_error}")
                    else:
                        scan_result_a, ocr_error_a = run_ocr_safely(reader, image_a, mode="nutrition")
                        if ocr_error_a:
                            st.error("OCR produk A gagal diproses.")
                        else:
                            parsed_data_a = scan_result_a["parsed"] if scan_result_a else {}
                            st.session_state.product_a_data['name'] = parsed_data_a.get('product_name') or st.session_state.product_a_data['name']
                            st.session_state.product_a_data['energi'] = float(parsed_data_a.get('energi') or st.session_state.product_a_data['energi'])
                            ...
                            st.session_state.product_a_data['natrium'] = float(parsed_data_a.get('natrium') or st.session_state.product_a_data['natrium'])
        ...
        p_a_takaran = st.number_input("Takaran Saji A (g)", min_value=1.0, value=float(data_a['takaran']), format="%.1f", key="takaran_a")
        p_a_energi = st.number_input("Energi A (kkal)", min_value=0.0, value=float(data_a['energi']), key="energi_a")
        p_a_lemak_total = st.number_input("Lemak Total A (g)", min_value=0.0, value=float(data_a['lemak_total']), format="%.1f", key="lemak_a")
        p_a_lemak_jenuh = st.number_input("Lemak Jenuh A (g)", min_value=0.0, value=float(data_a['lemak_jenuh']), format="%.1f", key="lemak_jenuh_a")
        p_a_protein = st.number_input("Protein A (g)", min_value=0.0, value=float(data_a['protein']), format="%.1f", key="protein_a")
        p_a_karbohidrat = st.number_input("Karbohidrat A (g)", min_value=0.0, value=float(data_a['karbohidrat']), format="%.1f", key="karbo_a")
        p_a_gula = st.number_input("Gula A (g)", min_value=0.0, value=float(data_a['gula']), format="%.1f", key="gula_a")
    ...
    with col2:
        ...
        if input_method_b == "Scan dengan Kamera":
            ...
            if img_file_b:
                ...
                with st.spinner("Memproses gambar Produk B..."):
                    # ==========================================================================
                    # 🌟 MARKER FITUR 1: EKSEKUSI OCR PRODUK B (LAZY LOAD & SAFE EXECUTION)
                    # ==========================================================================
                    reader, reader_error = get_ocr_reader_safely()
                    if reader_error:
                        st.error(f"Gagal memuat OCR: {reader_error}")
                    else:
                        scan_result_b, ocr_error_b = run_ocr_safely(reader, image_b, mode="nutrition")
                        if ocr_error_b:
                            st.error("OCR produk B gagal diproses.")
                        else:
                            parsed_data_b = scan_result_b["parsed"] if scan_result_b else {}
                            st.session_state.product_b_data['name'] = parsed_data_b.get('product_name') or st.session_state.product_b_data['name']
                            st.session_state.product_b_data['energi'] = float(parsed_data_b.get('energi') or st.session_state.product_b_data['energi'])
                            ...
                            st.session_state.product_b_data['natrium'] = float(parsed_data_b.get('natrium') or st.session_state.product_b_data['natrium'])
        ...
        p_b_takaran = st.number_input("Takaran Saji B (g)", min_value=1.0, value=float(data_b['takaran']), format="%.1f", key="takaran_b")
        p_b_energi = st.number_input("Energi B (kkal)", min_value=0.0, value=float(data_b['energi']), key="energi_b")
        p_b_lemak_total = st.number_input("Lemak Total B (g)", min_value=0.0, value=float(data_b['lemak_total']), format="%.1f", key="lemak_b")
        p_b_lemak_jenuh = st.number_input("Lemak Jenuh B (g)", min_value=0.0, value=float(data_b['lemak_jenuh']), format="%.1f", key="lemak_jenuh_b")
        p_b_protein = st.number_input("Protein B (g)", min_value=0.0, value=float(data_b['protein']), format="%.1f", key="protein_b")
        p_b_karbohidrat = st.number_input("Karbohidrat B (g)", min_value=0.0, value=float(data_b['karbohidrat']), format="%.1f", key="karbo_b")
        p_b_gula = st.number_input("Gula B (g)", min_value=0.0, value=float(data_b['gula']), format="%.1f", key="gula_b")
    ...
    if compare_button:
        ...
        # ==========================================================================
        # 🌟 MARKER FITUR 2: HASIL KLASIFIKASI PERBANDINGAN (VALIDASI ZERO-INPUT)
        # ==========================================================================
        if not has_sufficient_input(nutrition_a) or not has_sufficient_input(nutrition_b):
            st.error("Data produk A atau B belum lengkap! Harap masukkan data gizi yang valid sebelum membandingkan.")
        else:
            with st.spinner("Menganalisis dan membandingkan kedua produk..."):
                ...
                with res_col1:
                    ...
                    risk_info_a = classify_risk(risk_a)
                    st.metric(label="Skor Risiko AI", value=f"{risk_a:.2f}%", delta=risk_info_a["label"], delta_color="inverse" if risk_a > 25 else "normal")
                    st.metric(label="Kepadatan Energi", value=f"{kepadatan_a:.2f} kkal/g")

                with res_col2:
                    st.markdown(f"### {p_b_name}")
                    risk_info_b = classify_risk(risk_b)
                    st.metric(label="Skor Risiko AI", value=f"{risk_b:.2f}%", delta=risk_info_b["label"], delta_color="inverse" if risk_b > 25 else "normal")
                ...
                fig_compare_bar = px.bar(df_melted, x="Nutrisi (per 100g)", y="Kandungan", color="Produk", barmode="group", title=f"Perbandingan Nutrisi: {p_a_name} vs {p_b_name} (Distandarisasi per 100g)", text_auto='.1f')
    ...
    if len(st.session_state.scan_history) == 0:
        ...
        with col_g2: st.metric("Total Produk Dianalisis", str(total_scan))
        with col_g3: st.metric("Produk Berisiko Tinggi Ditemukan", str(high_risk_count), delta="-Kurangi konsumsi" if high_risk_count > 0 else "Aman", delta_color="inverse" if high_risk_count > 0 else "normal")

        st.markdown("---")
        st.subheader("📅 Calendar Heatmap: Aktivitas Pengecekan Nutrisi")
        history_df["day_date"] = pd.to_datetime(history_df["date"]).dt.date
        heatmap_data = history_df.groupby("day_date").agg(total_scans=("product_name", "count"), avg_score=("risk_score", "mean")).reset_index()
        heatmap_data["day_date"] = heatmap_data["day_date"].astype(str)

        fig_heat = px.density_heatmap(heatmap_data, x="day_date", y="total_scans", z="avg_score", color_continuous_scale="RdYlGn_r", title="Intensitas Pengecekan Harian (Warna = Tingkat Risiko Rata-rata)", labels={"day_date": "Tanggal", "total_scans": "Frekuensi Produk Di-Scan", "avg_score": "Skor Risiko"})
        ...
        col1, col2 = st.columns(2)
        with col1:
            fig_pie = px.pie(history_df, names="Kategori Risiko", title="Proporsi Kategori Risiko Produk", color="Kategori Risiko", color_discrete_map={"Sangat Tinggi": "darkred", "Tinggi": "red", "Sedang": "orange", "Rendah": "green"}, hole=0.4)
            st.plotly_chart(fig_pie, use_container_width=True)
        with col2:
            fig_line = px.line(history_df, x="date", y="risk_score", title="Tren Skor Risiko Konsumsi Seiring Waktu", markers=True, hover_data=["product_name"])
        ...
        scatter_df = pd.DataFrame({"Produk": history_df["product_name"], "Gula (g)": sugar_data, "Natrium (mg)": sodium_data, "Skor Risiko": history_df["risk_score"], "Kategori": history_df["Kategori Risiko"]})
        fig_scatter = px.scatter(scatter_df, x="Gula (g)", y="Natrium (mg)", size="Skor Risiko", color="Kategori", hover_name="Produk", title="Peta Risiko Berdasarkan Kandungan Gula dan Natrium", size_max=30, color_discrete_map={"Sangat Tinggi": "darkred", "Tinggi": "red", "Sedang": "orange", "Rendah": "green"})
    ...
    energi = c1.number_input("Energi (kkal)", min_value=0.0, value=150.0)
    ...
    natrium = c8.number_input("Natrium (mg)", min_value=0.0, value=45.0)
    natrium_benzoat = c9.number_input("Natrium Benzoat (mg)", min_value=0.0, value=0.0, format="%.2f")
    ...
    with freq_col: frequency_per_week = st.number_input("Frekuensi konsumsi per minggu (kali/sajian)", min_value=1, value=3)
    with period_col: simulation_period_months = st.selectbox("Periode Simulasi (Bulan)", [1, 3, 6, 12])
    ...
    if simulation_button:
        ...
        # ==========================================================================
        # 🌟 MARKER FITUR 2: HASIL KLASIFIKASI SIMULASI (VALIDASI ZERO-INPUT & CLASSIFY_RISK)
        # ==========================================================================
        if not has_sufficient_input(nutrition_data):
            st.error("⚠️ Form tidak boleh kosong untuk menjalankan simulasi konsumsi.")
        else:
            with st.spinner("Menjalankan simulasi konsumsi..."):
                risk_score, _, _ = analyze_product_fully(nutrition_data, komposisi, feat_model, lgbm_model, w2v_model, scaler)

                st.subheader(f"Hasil Analisis Dasar untuk '{product_name}'")
                res_col, _ = st.columns(2)
                with res_col:
                    risk_info = classify_risk(risk_score)
                    st.metric(label="Skor Risiko per 1x Konsumsi", value=f"{risk_score:.2f}%", delta=risk_info["label"], delta_color="inverse" if risk_score > 25 else "normal")
                # ================= END MARKER FITUR 2 =================
                ...
                if percent_gula > 100: st.error(f"🔴 Peringatan! Konsumsi produk ini saja sudah **melebihi {percent_gula - 100:.0f}%** dari batas aman gula Anda.")
                elif percent_gula > 50: st.warning(f"🟡 Perhatian. Konsumsi produk ini menggunakan **{percent_gula:.0f}%** dari alokasi gula Anda untuk periode ini.")
                ...
                if percent_natrium > 100: st.error(f"🔴 Peringatan! Konsumsi produk ini saja sudah **melebihi {percent_natrium - 100:.0f}%** dari batas aman natrium Anda.")
                elif percent_natrium > 50: st.warning(f"🟡 Perhatian. Konsumsi produk ini menggunakan **{percent_natrium:.0f}%** dari alokasi natrium Anda untuk periode ini.")
                ...
                if percent_lemak_jenuh > 100: st.error(f"🔴 Peringatan! Konsumsi produk ini saja sudah **melebihi {percent_lemak_jenuh - 100:.0f}%** dari batas aman lemak jenuh Anda.")
                elif percent_lemak_jenuh > 50: st.warning(f"🟡 Perhatian. Konsumsi produk ini menggunakan **{percent_lemak_jenuh:.0f}%** dari alokasi lemak jenuh Anda untuk periode ini.")
